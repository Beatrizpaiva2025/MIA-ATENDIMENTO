    ...
    try:
        ...
        for c in conversoes:
            ...
            result.append({
                ...
                "tipo_atendimento": "Human + AI" if teve_humano else "AI",
    ...
    try:
        ...
        for phone in leads_nao_convertidos[:30]:
            ...
            pediu_orcamento = await db.conversas.find_one({
                "phone": phone,
                "message": {"$regex": "quote|price|cost|how much", "$options": "i"}
            })
            ...
            result.append({
                "phone": phone,
                "ultima_msg": ultima_msg.get("timestamp").strftime("%m/%d/%Y %H:%M") if ultima_msg and ultima_msg.get("timestamp") else "N/A",
                "total_msgs": total_msgs,
                "dias_sem_contato": dias_sem_contato,
                "pediu_orcamento": "Yes" if pediu_orcamento else "No",
                "prioridade": "High" if pediu_orcamento and dias_sem_contato <= 3 else ("Medium" if pediu_orcamento else "Low"),
...
async def api_registrar_conversao(request: Request):
    ...
    try:
        ...
        logger.info(f"Conversion saved: {data.get('phone')} - $ {data.get('valor')}")

        return {"success": True, "message": "Conversion saved successfully"}
    except Exception as e:
...
async def api_get_resumo():
    ...
    try:
        ...
        return {
            "today": {
                "conversions": conversoes_hoje,
                "value": valor_hoje[0]["total"] if valor_hoje else 0
            },
            "week": {
                "conversions": conversoes_semana,
                "value": valor_semana[0]["total"] if valor_semana else 0
            },
            "month": {
                "conversions": conversoes_mes,
                "value": valor_mes[0]["total"] if valor_mes else 0
            }
        }
    except Exception as e:
